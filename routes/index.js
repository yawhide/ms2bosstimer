...
router.get('/', function(req, res, next) {
  res.render('index', { title: 'MS2 Boss Timers', times: JSON.stringify(getNextSpawn()) });
});
...
function updateTimes(){
  ...
  // for (var i = 0; i < griffyTimersEst.length; i++) {
  //   console.log(griffyTimersEst[i].toDate())
  // };
  // setTimeout(updateTimes, 1*60*1000)
}

function increaseDay(arrayOfTimes){
  var now = moment()
  for (var i = 0; i < arrayOfTimes.length; i++) {
    if(now.isAfter(arrayOfTimes[i])){
